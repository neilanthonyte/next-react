...
import { PayDockCreditCardFormCtrl } from "./index.ctrl";

fixture("PayDockCreditCardForm").page(
  "http://localhost:6060/#/PayDockCreditCardForm",
);
 ...
 *
 * ```
 * PATTERN=./src/components/payments/PayDockCreditCardForm/index.tsx yarn dev
 * yarn testcafe ./src/components/payments/PayDockCreditCardForm/_tests/index.intg.ts
 * ```
 */
test("something happens", async (t) => {
  const selector = Selector(selectDemo("PayDockCreditCardForm", "standard"));

  const component = new PayDockCreditCardFormCtrl(
