...
import { FoyerScreensCtrl } from "./index.ctrl";

fixture("FoyerScreens").page("http://localhost:6060/#/FoyerScreens");

/**
 * To run:
 *
 * ```
 * PATTERN=./src/components/foyer/FoyerScreens/index.tsx yarn dev
 * yarn testcafe ./src/components/foyer/FoyerScreens/_tests/index.intg.ts
 * ```
 */
test("something happens", async (t) => {
  const selector = Selector(selectDemo("FoyerScreens", "standard"));

  const component = new FoyerScreensCtrl(selector.find(selectComponent()), t);
