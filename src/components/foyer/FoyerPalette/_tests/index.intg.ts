...
import { FoyerPaletteCtrl } from "./index.ctrl";

fixture("FoyerPalette").page("http://localhost:6060/#/FoyerPalette");

/**
 * To run:
 *
 * ```
 * PATTERN=./src/components/foyer/FoyerPalette/index.tsx yarn dev
 * yarn testcafe ./src/components/foyer/FoyerPalette/_tests/index.intg.ts
 * ```
 */
test("something happens", async (t) => {
  const selector = Selector(selectDemo("FoyerPalette", "standard"));

  const component = new FoyerPaletteCtrl(selector.find(selectComponent()), t);
