...
import { FoyerAppCtrl } from "./index.ctrl";

fixture("FoyerApp").page("http://localhost:6060/#/FoyerApp");

/**
 * To run:
 *
 * ```
 * PATTERN=./src/components/foyer/FoyerApp/index.tsx yarn dev
 * yarn testcafe ./src/components/foyer/FoyerApp/_tests/index.intg.ts
 * ```
 */
test("something happens", async (t) => {
  const selector = Selector(selectDemo("FoyerApp", "standard"));

  const component = new FoyerAppCtrl(selector.find(selectComponent()), t);
