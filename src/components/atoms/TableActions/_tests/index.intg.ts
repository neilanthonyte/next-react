...
import { TableActionsCtrl } from "./index.ctrl";

fixture("TableActions").page("http://localhost:6060/#/TableActions");

/**
 * To run:
 *
 * ```
 * PATTERN=./src/components/atoms/TableActions/index.tsx yarn dev
 * yarn testcafe ./src/components/atoms/TableActions/_tests/index.intg.ts
 * ```
 */
test("something happens", async (t) => {
  const selector = Selector(selectDemo("TableActions", "standard"));

  const component = new TableActionsCtrl(selector.find(selectComponent()), t);
