...
import { NextPatientUpcomingAppointmentsCtrl } from "./index.ctrl";

fixture("NextPatientUpcomingAppointments").page(
  "http://localhost:6060/#/NextPatientUpcomingAppointments",
);
 ...
 *
 * ```
 * PATTERN=./src/components/atoms/NextPatientUpcomingAppointments/index.tsx yarn dev
 * yarn testcafe ./src/components/atoms/NextPatientUpcomingAppointments/_tests/index.intg.ts
 * ```
 */
test("something happens", async (t) => {
  const selector = Selector(
    selectDemo("NextPatientUpcomingAppointments", "standard"),
  );

  const component = new NextPatientUpcomingAppointmentsCtrl(
