...
import { NavBarCtrl } from "./index.ctrl";
// import { demoIsLoading } from "../../../helpers/demoIsLoading";

fixture("NavBar").page("http://localhost:6060/#/NavBar");

/**
 * To run:
 *
 * ```
 * PATTERN=./src/components/provider-app/NavBar/index.tsx yarn dev
 * yarn testcafe ./src/components/provider-app/NavBar/_tests/index.intg.ts
 * ```
 */
test("something happens", async (t) => {
  const selector = Selector(selectDemo("NavBar", "standard"));

  const component = new NavBarCtrl(selector.find(selectComponent()), t);
