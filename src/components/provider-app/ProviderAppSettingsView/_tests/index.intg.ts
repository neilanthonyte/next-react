...
import { ProviderAppSettingsViewCtrl } from "./index.ctrl";

fixture("ProviderAppSettingsView").page(
  "http://localhost:6060/#/ProviderAppSettingsView",
);
 ...
 *
 * ```
 * PATTERN=./src/components/provider-app/ProviderAppSettingsView/index.tsx yarn dev
 * yarn testcafe ./src/components/provider-app/ProviderAppSettingsView/_tests/index.intg.ts
 * ```
 */
test("something happens", async (t) => {
  const selector = Selector(selectDemo("ProviderAppSettingsView", "standard"));

  const component = new ProviderAppSettingsViewCtrl(
