...
import { StepThroughCtrl } from "./index.ctrl";

fixture("StepThrough").page("http://localhost:6060/#/StepThrough");

/**
 * To run:
 *
 * ```
 * PATTERN=./src/components/structure/StepThrough/index.tsx yarn dev
 * yarn testcafe ./src/components/structure/StepThrough/_tests/index.intg.ts
 * ```
 */
test("something happens", async (t) => {
  const selector = Selector(selectDemo("StepThrough", "standard"));

  const component = new StepThroughCtrl(selector.find(selectComponent()), t);
