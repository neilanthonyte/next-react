...
import { TwoFactorAuthenticationFormCtrl } from "./index.ctrl";
import { demoIsLoading } from "../../../../helpers/demoIsLoading";

fixture("TwoFactorAuthenticationForm").page(
  "http://localhost:6060/#/TwoFactorAuthenticationForm",
);
 ...
 *
 * ```
 * PATTERN=./src/components/forms/TwoFactorAuthenticationForm/index.tsx yarn dev
 * yarn testcafe ./src/components/forms/TwoFactorAuthenticationForm/_tests/index.intg.ts
 * ```
 */
test("something happens", async (t) => {
  const selector = Selector(
    selectDemo("TwoFactorAuthenticationForm", "standard"),
  );

  const component = new TwoFactorAuthenticationFormCtrl(
