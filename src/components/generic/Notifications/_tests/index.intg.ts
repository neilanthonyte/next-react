...
import { NotificationsCtrl } from "./index.ctrl";

fixture("Notifications").page("http://localhost:6060/#/Notifications");

/**
 * To run:
 *
 * ```
 * PATTERN=./src/components/generic/Notifications/index.tsx yarn dev
 * yarn testcafe ./src/components/generic/Notifications/_tests/index.intg.ts
 * ```
 */
test("something happens", async (t) => {
  const selector = Selector(selectDemo("Notifications", "standard"));

  const component = new NotificationsCtrl(selector.find(selectComponent()), t);
