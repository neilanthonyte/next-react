...
import { ChoiceCtrl } from "./index.ctrl";

fixture("Choice").page("http://localhost:6060/#/Choice");

/**
 * To run:
 *
 * ```
 * PATTERN=./src/components/companion/Choice/index.tsx yarn dev
 * yarn testcafe ./src/components/companion/Choice/_tests/index.intg.ts
 * ```
 */
test("something happens", async (t) => {
  const selector = Selector(selectDemo("Choice", "standard"));

  const component = new ChoiceCtrl(selector.find(selectComponent()), t);
